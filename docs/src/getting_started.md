...
Build temporal observations, compute a layout sequence, and export an animation. The tutorial then explores interpolation and an illustrative infection display; displayed attributes are supplied data, not an epidemic model fitted by NDTV.jl.
...
With `FRLayout()` and `anchor=true`, each frame starts from the previous frame's positions before force-directed updates. Reusing these coordinates can reduce visual jumps; it does not fix positions or guarantee a smooth result:
...
Anchoring affects the force-directed initialization. `CircleLayout()` and
`MDSLayout()` recompute their deterministic layouts, and layout movement
should not be interpreted as physical motion or changing social distance.
...
| Algorithm | What determines positions | Across frames |
|-----------|---------------------------|---------------|
| `FRLayout` | Iterative attractive and repulsive forces | Optional initialization from previous positions; inspect movement |
| `MDSLayout` | Classical MDS of geodesic distances | Recomputed; changing distances can rotate or move the drawing |
| `CircleLayout` | Vertex order on a circle | Fixed for the same retained actor set and parameters |
| `RandomLayout` | Uniform random coordinates | Seed the RNG; anchoring keeps existing positions |
...
```julia
layout = render_animation(dnet; n_frames=100, rng=Xoshiro(3))
...
layout = render_animation(dnet; n_frames=100, algorithm=FRLayout(), rng=Xoshiro(4))
...
2. **Compare anchored layouts**: For force-directed layouts, compare frames with `anchor=true` and inspect large position changes
3. **Choose frames from the data**: Include times that capture short activity spells; visual smoothness alone does not establish temporal coverage
